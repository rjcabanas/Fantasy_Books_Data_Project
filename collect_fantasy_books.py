...
def save_api_page(filename:str, page_number:int, data):
       with open(f"raw/fantasy_books_library/{filename}_page_{page_number}.json", "w") as f:
...
def create_book_library_file(folder_path:str):
       ...
       df_combined_book_libary.to_parquet("bronze\\fantasy_book_library.parquet", index=False)

def collect_fantasy_books_bronze():
       collect_all_fantasy_books(fantasy_books_url)
       create_book_library_file("raw\\fantasy_books_library")
